...
# is var does not exist, define it and set de default value
...
export KAFKA__LOG_DIRS=${KAFKA__LOG_DIRS:-"/opt/kafka/logs"}
...
if [ -f $KAFKA_SERVER_CONFIG_FILE ]; then
  ...
  echo "#### FILE MANAGED BY global-functions.sh SCRIPT ####" > $KAFKA_SERVER_CONFIG_FILE
fi
